...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, 1024, 768 - 64) style:UITableViewStylePlain];
...
{
    return ((_tableData.count / 4) > 3 ? _tableData.count/4 : 3);
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    static NSString * cellID = @"ShelfItemID";
    ShelfViewCell * cell = (ShelfViewCell*)[_tableView dequeueReusableCellWithIdentifier:cellID];
    if (cell == nil)
    {
        cell = [[[ShelfViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID] autorelease];
        NSArray * nib = [[NSBundle mainBundle] loadNibNamed:@"ShelfViewCell" owner:nil options:nil];
        cell = (ShelfViewCell*)[nib objectAtIndex:0];
        [cell setSelectionStyle:UITableViewCellEditingStyleNone];
    }
    
    for (int i = 0; i < 4; i++) {
        if (indexPath.row * 4 + i < _tableData.count)
        {
            NSMutableDictionary * lessonDict = [[NSMutableDictionary alloc] init];
            lessonDict = [[_tableData objectAtIndex:(indexPath.row * 4 + i)] retain];
...
{
    ShelfViewCell * shellRow = cell;
    NSIndexPath * indexPath = [_tableView indexPathForCell:shellRow];
    
    NSInteger cellIndex = indexPath.row * 4 + columnIndex;
    ...
    if ([[lessonDict valueForKey:@"lessonType"] isEqualToString:LessonCategory])
    {
        ...
        [categoryDetailView setTitle:[lessonDict valueForKey:@"lessonName"]];
